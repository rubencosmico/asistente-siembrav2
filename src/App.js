...
/**
 * Coordenadas geográficas para el área objetivo (Elche, Alicante).
 */
const CONFIG = {
    LATITUDE: 38.27,
    LONGITUDE: -0.70,
...
/**
 * Umbrales de lluvia (en mm) para determinar la viabilidad de la siembra.
 * Ver docs/BUSINESS_LOGIC.md para una explicación detallada.
 */
const THRESHOLDS = {
    PERFECT: 90.0,
    OPTIMAL: 70.0,
...
/**
 * Criterios para definir un "Día de Lluvia Significativo" en el pronóstico.
 */
...
/**
 * Componente: Tarjeta de Pronóstico Diario
 * Muestra el pronóstico para un día específico.
 */
...
/**
 * Componente: Panel en Tiempo Real (Live Dashboard)
 * Controlador lógico principal. Obtiene datos, calcula índices y muestra la recomendación.
 */
const LiveDashboard = () => {
    ...
    useEffect(() => {
        const fetchWeatherData = async () => {
            ...
            // Construcción de la solicitud a la API
            ...
            try {
                ...
                // Invocar al Motor de Recomendación
        ...
        /**
         * Lógica Central: Procesa los datos meteorológicos para generar una recomendación de siembra.
         * @param {Object} data - La respuesta JSON de la API Open-Meteo.
         */
        const processAndRecommend = (data) => {
            // 1. Calcular Lluvia Acumulada (Últimos 7 Días)
            // Esto representa la reserva actual de humedad del suelo.
            ...
            // Identifica "Lluvia de Seguimiento" para asegurar la supervivencia de la plántula.
            ...
            for (let i = 0; i < forecastRain.length; i++) {
                // Comprobar criterios para un "Día de Lluvia Significativo"
            ...
            // 3. Motor de Reglas de Recomendación
            // Ver docs/BUSINESS_LOGIC.md para la matriz de decisión.
            ...
            if (sevenDayTotal >= THRESHOLDS.PERFECT) {
                ...
                // Comprobación Crítica: En el rango medio, la lluvia de seguimiento es obligatoria en 5 días.
                if (daysToNextRain <= 5) {
                    rec.details += ` Se esperan lluvias en ${daysToNextRain} día(s).`;
                } else {
                    // Degradar a Arriesgada si no hay lluvia de seguimiento
...
// --- Componente: Datos Históricos (Hardcoded) ---
...
const SoilProfileCard = () => (
    <div className="bg-amber-50 rounded-xl shadow-md p-6 border border-amber-200 mb-8">
        <h2 className="text-2xl font-bold mb-4 flex items-center text-amber-900">
            <Mountain className="w-7 h-7 mr-3 text-amber-700" />
            Perfil del Suelo: Aridisol (Zona de Elche)
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-gray-700">
            <div className="md:col-span-1">
                <h4 className="font-bold text-lg mb-2">Características Clave:</h4>
                <ul className="list-disc list-inside space-y-1">
                    <li>Baja materia orgánica.</li>
                    <li>Acumulación de sales (carbonatos).</li>
                    <li>Baja retención de agua.</li>
                    <li>Riesgo de costra superficial.</li>
                </ul>
            </div>
            <div className="md:col-span-2">
                <h4 className="font-bold text-lg mb-2">Implicaciones para la Siembra:</h4>
                <p className="mb-2"><span className="font-bold text-red-600">Siembra Arriesgada (+{THRESHOLDS.MINIMUM}mm):</span> Humedad superficial. Viable solo si hay lluvias de seguimiento inminentes.</p>
                <p className="mb-2"><span className="font-bold text-green-700">Siembra Segura (+{THRESHOLDS.OPTIMAL}mm):</span> La lluvia penetra lo suficiente para superar la zona de alta evaporación, creando una reserva de humedad.</p>
                <p><span className="font-bold text-blue-800">Siembra Perfecta (+{THRESHOLDS.PERFECT}mm):</span> Garantiza una saturación profunda del perfil del suelo. Es la ventana ideal que maximiza las probabilidades de supervivencia a largo plazo.</p>
            </div>
        </div>
    </div>
);
